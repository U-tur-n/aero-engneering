...
\subsection{本課題で用いる条件}
求めるにあたって用いた定数を表\ref{table:定数}に示す．なお，大気の状態については標準大気表を参考にした\cite{標準大気}．
\begin{table}[hbtp]
\centering
\caption{}
\label{table:定数}
...
風洞試験では，実機と等しい条件を用意することが難しいため，小さいサイズの模型を用いなければならない．小さいサイズの模型を用いると，測定される揚力と抗力は実機と比較して異なる値を示すため，そのデータを直接用いることはできない．そのため，揚力と抗力を無次元化した，揚力係数と抗力係数を算出する必要がある．
...
\end{align}
このうち，$C_L$については，水平定常飛行をしていると仮定すると，巡行時の揚力$L_c$と重量$Wg$のつり合いから，以下のように表される．ただし，$g$は重力加速度であり，$g = 9.80665$ [m/s$^2$]である．
\begin{align}
  \begin{split}
\label{eq:巡行揚力係数}
...
\subsection{風洞試験で得られる揚力係数${C_L}_0$と抗力係数$C_{D_0}$}
さて，必要なパラメータがそろったため，風洞試験時の揚力係数$C_{L_0}$と抗力係数$C_{D_0}$を算出する．風洞試験時の揚力と抗力は，実際に測定を行う必要があるため，ここでは揚力と抗力をそれぞれ$L_0$，$D_0$と表す．これらの値を用いて，風洞試験時の揚力係数$C_{L_0}$と抗力係数$C_{D_0}$を算出する．
\eqref{eq:揚力係数}より，風洞試験時の揚力係数$C_{L_0}$は以下のようになる．
\begin{align}
\label{eq:風洞揚力係数}
C_{L_0} = \frac{L_0}{\frac{1}{2}\rho_0' {U_0'}^2 S_0} = \frac{L_0}{\frac{1}{2} \cdot 11.89 \cdot 268.7^2 \cdot 5.148}
\end{align}
同様に，風洞試験時の抗力係数$C_{D_0}$は以下のようになる．
\begin{align}
\label{eq:風洞抗力係数}
C_{D_0} = \frac{D_0}{\frac{1}{2}\rho_0' U_0'^2 S_0} = \frac{D_0}{\frac{1}{2} \cdot 11.89 \cdot 268.7^2 \cdot 5.148}
\end{align}
ここで，冒頭で求めた巡行時の揚力係数$C_{L_c}$と風洞試験時の揚力係数$C_{L_0}$は同じ値を示す必要がある．したがって，\eqref{eq:巡行揚力係数}と\eqref{eq:風洞揚力係数}を用いて，風洞試験時の揚力$L_0$を求めると以下のようになる．
\begin{align}
\begin{split}
\label{eq:風洞揚力}
L_0 &= C_{L_0} \cdot \frac{1}{2} \cdot \rho_0' \cdot {U_0'}^2 \cdot S_0 \\
&= 0.04765 \cdot \frac{1}{2} \cdot 11.89 \cdot 268.7^2 \cdot 5.148 \\
&= 1.053 \times 10^5 \mathrm{N}
\end{split}
\end{align}

その後，$L_0$と同じ値の揚力を得られるような迎角を風洞試験により算出し，その迎角で揚力$L_0$を水平垂直に分解し，水平の揚力成分を抗力$D_0$とする．算出された$D_0$を\ref{eq:風洞抗力係数}に代入することで，抗力係数を求めることができる．

このようにして求まった揚力係数と抗力係数を用いて，実機の揚力および抗力を計算することができる．

\section{数値流体力学を用いた模型の風洞試験を用いた揚力$L$，抗力$D$の求め方}
ナヴィエ・ストークス方程式は非線形の偏微分方程式であるため，解析的に解くことは困難である．そのため，数値流体力学(CFD)を用いて数値的に解く必要がある．具体的には，細かく分割したメッシュ上で数値的に解く．
\subsection{モデル化}
実機の全抗力を精度よく求めるためには、空気の粘性と圧縮性の両方を考慮する必要がある．そのため，具体的には、粘性による摩擦抗力や境界層の剥離、および巡航マッハ数 $M=0.79$ で発生する衝撃波に伴う造波抗力を計算できるモデルを用いる．

\subsection{計算コストと実用性}
実機全体の3次元解析を行うと，格子点数が膨大になり，計算コストが高くなってしまう．そのため，まずは揚力線理論など，簡略的に求められる線形モデルで大まかな計算を行い，その後詳細な形状を含む機体全体のCFD解析を行うといった，段階的なステップを踏む必要がある．

\subsection{揚力$L$，抗力$D$の算出}
シミュレーションを実行し，収束した計算結果から，圧力係数$C_p$分布を求める．求めた$C_p$を積分し，物体に働く力を求める．この力の成分を分解し，垂直に働く力を揚力$L$，水平に働く力を抗力$D$として，式\ref{eq:揚力係数}，\ref{eq:抗力係数}を用いて無次元化する．

このようにして求まった揚力係数$C_L$，抗力係数$C_D$を用いて，実機の揚力と抗力を求める．実機の揚力$L$，抗力$D$を求めるにあたって用いる文字には添え字としてcを付与する．
\begin{align}
\label{eq:}
L = \frac{1}{2}\rho_c {U_c}^2 S_c C_L
D = \frac{1}{2}\rho_c {U_c}^2 S_c C_D
\end{align}
...
%bibitem{キー} 著者 『題名』，url{URL}，(閲覧日)
  ...
  \bibitem{標準大気} HEISHIN 『標準大気(高度2,0000m迄)』, \url{https://www.eng-book.com/pdfs/a5cee5b2ba4081d74c629e07908ffe18.pdf} (閲覧日：2026年6月10日)
\end{thebibliography}
